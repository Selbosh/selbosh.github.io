...
This is basically like the convolution we applied in [day 11](#day11), but with an
extra dimension. In R, an array is a matrix with an arbitrary number of
dimensions. We can also take advantage of the outer product `%o%` to pad
the space with zeros initially.
