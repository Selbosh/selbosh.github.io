...
recursive_recursive_combat <- function(p1, p2, played = NULL) {
  ...
  if (p1[1] < length(p1) & p2[1] < length(p2)) {
    ...
    win1 <- if (max(p1[-1]) > max(p2[-1])) TRUE else
      all( recursive_recursive_combat(head(p1[-1], p1[1]),
                                      head(p2[-1], p2[1])) > 0 )
  } else win1 <- p1[1] > p2[1]
...
recursion was not such a great idea. My original implementation of this
caused a stack overflow, until I patched in the player 1 sub-game trick
(described below).
...
recursive_combat <- function(p1, p2, played = NULL) {
  while (length(p1) & length(p2)) {
    ...
    if (p1[1] < length(p1) & p2[1] < length(p2)) {
      win1 <- if (max(p1[-1]) > max(p2[-1])) TRUE else
        all( recursive_combat(head(p1[-1], p1[1]),
                              head(p2[-1], p2[1])) > 0 )
    } else win1 <- p1[1] > p2[1]
...
Writing recursive functions just to avoid loops is a bad idea because it
...
However, it turns out that my original nested recursion actually works
without error if I exploit a trick—that I missed first time around—to
skip simulating many of the sub-games.
...
computation can be avoided, and it even reduces the call stack such that
`recursive_recursive_combat()` works without error (at least on my input
dataset).

According to `microbenchmark`, this trick cuts my run time from around
34 whole seconds to just 15 milliseconds (for either the loop or the
double recursion). Nice optimization when you can get it!
