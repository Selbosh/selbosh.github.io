...
purposes. We’ll call it `Bisamd` to represent our new order.

``` r
Bisamd <- function(expr) {
...
``` r
results2 <- sapply(homework, Bisamd)
...
Reassigning addition to another function is also a hilarious prank you can play
on someone who never clears out their global workspace. It'll teach them the
value of starting with clean working environment.
