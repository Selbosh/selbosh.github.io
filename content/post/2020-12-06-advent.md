...
has $m$ cards, we can simulate the first $\min(n, m)$ rounds in parallel.
...
1. If this exact round has happened before (during *this game*), we stop and
   player 1 automatically wins the *game*, to prevent an infinite recursion.
2. If each player's deck has more cards than the `value` of their latest draw,
   copy the next `value` cards and play a (recursive) sub-game to determine the
...
configuration as a string. However, you could also take a hint from submitting
the answer to part 1: the sum of the deck multiplied by its indices is unique.
...
``` r
recursive_recursive_combat <- function(p1, p2, played = NULL) {
  if (!length(p1)) return(-p2)
  if (!length(p2)) return(p1)
  ...
  recursive_recursive_combat(result1, result2, played = played)
}
```
...
``` r
example2 <- recursive_recursive_combat(ex1, ex2)
sum(rev(example2) * seq_along(example2))
```

    [1] -291

But on the test inputs, it becomes clear that a recursion within a
recursion was not such a great idea. Stack overflow!

``` r
recursive_recursive_combat(deck[, 1], deck[, 2])
```

    Error in stopifnot(length(result1) + length(result2) == length(p1) + length(p2)) : 
      node stack overflow
    Error during wrapup: node stack overflow
    Error: no more error handlers available (recursive errors?); invoking 'abort' restart
    
Rewriting the main game as a loop, everything works as designed.

``` r
recursive_combat <- function(p1, p2, played = NULL) {
  while (length(p1) & length(p2)) {
    ...
    result1 <- c(p1[-1], p1[1][win1], p2[1][win1])
    result2 <- c(p2[-1], p2[1][!win1], p1[1][!win1])
    stopifnot(length(result1) + length(result2) == length(p1) + length(p2))
...
It turns out that writing recursive functions instead of loops is
neither big nor clever, and I have certainly learnt my lesson.

Also: it looks like the crab lost the final game.
