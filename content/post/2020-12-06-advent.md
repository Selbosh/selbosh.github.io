...
I could have equally appended the 3 in the final line as
`c(diff(jolts), 3)`.
...
1. if *n* = 1 the adapter can’t be removed because the gap would then
    be 4:
    - ${0 \choose 0} = 1$
2. if *n* = 2 then you can only remove the first adapter (or not):
    - ${1 \choose 0} + {1 \choose 1} = 2$
3. if *n* = 3 you can keep them all, remove 1 or both of the first 2:
    - ${2 \choose 0} + {2 \choose 1} + {2 \choose 2} = 4$
4. if *n* = 4 you can keep them all, or remove *up to 2* of the first
    3:
    - ${3 \choose 0} + {3 \choose 1} + {3 \choose 2} = 7$
5. and so on (though actually there aren’t any sequences longer than 4)
