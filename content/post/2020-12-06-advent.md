...
line_of_sight <- apply(seat_ids, 1, function(ij) {
  visible_seats <- NULL
  for (d in 1:nrow(dirs)) {
    radius <- 1
    while (TRUE) {
      i <- ij[1] + radius * dirs$right[d]
      j <- ij[2] + radius * dirs$down[d]
      
      if (i < 1 | j < 1 | i > max(seat_ids[, 1]) | j > max(seat_ids[, 2])) {
        break
      }
      
      if ( any(seat_ids[, 'row'] == i & seat_ids[, 'col'] == j) ) {
        visible_seats <- rbind(visible_seats, c(i, j))
        break
      }
...
This was pretty slow, which is to be expected in R. To speed it up,
we can rewrite the guts in a lower-level programming language. There
may also be some scope for vectorisation.
