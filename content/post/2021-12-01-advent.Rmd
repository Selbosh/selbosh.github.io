...
$$
for any launch velocity $u$, the peak of the curve is $\max\{0, u_y(u_y + 1) / 2\}$ where $u_y$ is the vertical component of $u$.
...
- It takes $t = u_x(u_x+1)/2$ time points for the probe to be slowed down by drag to zero horizontal velocity. This gives us a lower bound $$u_x > \frac12 \left(\sqrt{8x_1+1} - 1\right),$$ because if the initial is any less than this then the probe will never reach the left edge of the target area.
