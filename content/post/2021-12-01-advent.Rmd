...
This allows me to define a new `Cuboid` class, with fields describing the `x`, `y` and `z` ranges, and to write methods to, if an instance of a cuboid is overlapping with another, how to divide the two into a set of non-overlapping ones to perform the necessary set operations.
...
In other words, the penulatimate expression must be `TRUE`: the top value of the stack, plus 11, is equal to the most recent input value (`w`).
